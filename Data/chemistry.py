...
def main():
    ...
    user_imput = input('Enter the molecular formula of the sample: \n>')
    mass_in_grams = float(input('Enter the mass in grams of the sample: \n>'))
    dictionary = make_periodic_table()
    formula = parse_formula(user_imput, dictionary)

    molar = compute_molar_mass(formula, dictionary)

    moles = mass_in_grams / molar

    print(f'{round(molar,5)} grams/mole')
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for i in symbol_quantity_list:
        ...
        get_atomic_mass = quantity * atomic_mass
        total_mass += get_atomic_mass
